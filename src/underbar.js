(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(item, index) {
      resultArr.push(iterator(item, index, collection));
    });
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    if (accumulator !== undefined) {
      ...
      newCollection = _.last(collection, collection.length - 1);
    }
  ...
  _.extend = function(obj) {
    _.each(arguments, function(eachObj) {
      _.each(eachObj, function(sourceVal, sourceKey) {
        obj[sourceKey] = sourceVal;
      });
    });
  ...
  _.shuffle = function(array) {
    ...
    for (var i = Math.floor(Math.random() * arrayCopy.length); arrayCopy.length > 0; i = Math.floor(Math.random() * arrayCopy.length)) {
      retArray.push(arrayCopy[i]);  
